...
console.log("Mano detectada");
    ...
    const indiceArriba = d(hand[8], hand[5]) > 0.1;
    const medioArriba = d(hand[12], hand[9]) > 0.1;
    const anularArriba = d(hand[16], hand[13]) > 0.1;
    const meñiqueArriba = d(hand[20], hand[17]) > 0.1;

    const dedosArriba = [indiceArriba, medioArriba, anularArriba, meñiqueArriba].filter(v => v).length;

    // 🖐️ Mano abierta
    if (dedosArriba >= 4) {
        console.log("🖐️ Mano abierta → Agregar comanda");
        agregarComanda();
        return;
    }
    ...
    // ✊ Puño cerrado
    if (dedosArriba === 0) {
        console.log("✊ Puño cerrado → Iniciar temporizador");
        const comandas = document.querySelectorAll(".comanda");
        ultimaComanda = comandas[comandas.length - 1];
        if (ultimaComanda) {
            const boton = ultimaComanda.querySelector("button");
    ...
    // ☝️ Un dedo
    if (dedosArriba === 1 && indiceArriba) {
        console.log("☝️ Dedo índice → Detener comanda");
        if (ultimaComanda) detenerTemporizador(ultimaComanda);
        return;
    }
});
